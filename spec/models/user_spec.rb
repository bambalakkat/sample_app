...
describe User do
  ...
  before(:each) do
    @attr = { :name => 'Example User', :email => 'exampleuser@example.com'}
  end
  ...
  it "should reject a name that are too long" do
    long_name = "a" * 51
  ...
  it "should reject invalid email adresses" do
    addresses = %w[user.foo.com THE_USER@org. first.last_foo,jp]
    addresses.each do |address|
  ...
  it "should reject duplicate email adresses identical up to case" do
